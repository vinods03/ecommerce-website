...
export class CheckoutComponent implements OnInit {
  ...
  ngOnInit(): void {
        ...
        creditCard: this.formBuilder.group(
          {
            ...
            creditCardCVV: new FormControl('',[Validators.required, Validators.pattern('[0-9]{3}')]),
            creditCardName: new FormControl('',[Validators.required, Validators.minLength(2)]),
    ...
    this.cartService.getCountries().subscribe(
        data => {
          this.countries = data
        }
      )
    
    this.cartService.getMonths().subscribe(
      data => {
        this.months = data
      }
    )

    this.cartService.getYears().subscribe(
      data => {
        this.years = data
      }
    )
  ...
  getStatesByCountry() {
    ...
    this.cartService.getStatesByCountry(country_id).subscribe(
      data => {
        this.states = data
      }
    )
  ...
  onSubmit() {
    ...
    this.cartService.createOrder(purchase).subscribe(
      data => {
